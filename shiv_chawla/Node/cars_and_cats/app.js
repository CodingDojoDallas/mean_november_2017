...
var server = http.createServer(function (request, response){
    ...
    if(request.url === '/') {
      ...
      else if(request.url === '/images/solara.jpg'){
        // notice we won't include the utf8 encoding
        fs.readFile('./images/solara.jpg', function(errors, contents){
            response.writeHead(200, {'Content-type': 'image/jpg'});
            response.write(contents);
            response.end();
        })
      }
      else if(request.url === '/images/nala.jpg'){
        // notice we won't include the utf8 encoding
        fs.readFile('./images/nala.jpg', function(errors, contents){
            response.writeHead(200, {'Content-type': 'image/jpg'});
            response.write(contents);
            response.end();
        })
